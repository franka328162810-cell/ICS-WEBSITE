...
(function () {
  ...
  style.textContent = [
    ...
    '.lg-backdrop { position:absolute; top:0; left:0; width:100%; height:100%; background:#0a1628; }',
    '.lg-modal { position:relative; z-index:1; max-width:520px; width:90%; background:rgba(15,25,50,0.98); border:1px solid rgba(167,139,250,0.2); border-radius:16px; padding:2.5rem; text-align:center; box-shadow:0 20px 60px rgba(0,0,0,0.6); }',
    ...
    '.lg-subtitle { font-size:1rem; color:rgba(255,255,255,0.6); margin-bottom:0.3rem; }',
    ...
    '.lg-section h2 { font-size:1rem; font-weight:600; color:#f1f5f9; margin-bottom:0.75rem; }',
    ...
    '#lg-activate-btn { padding:0.75rem 1.5rem; background:' + info.color + '; color:#0a1628; border:none; border-radius:8px; font-weight:600; font-size:0.9rem; cursor:pointer; transition:opacity 0.2s; white-space:nowrap; }',
    '#lg-activate-btn:hover { opacity:0.85; }',
    '.lg-error { color:#ef4444; font-size:0.85rem; margin-top:0.75rem; }',
    '.lg-buy-btn { display:inline-block; padding:0.875rem 2rem; background:linear-gradient(135deg,' + info.color + ',rgba(167,139,250,0.8)); color:#fff; text-decoration:none; border-radius:10px; font-weight:600; font-size:0.95rem; transition:transform 0.2s,box-shadow 0.2s; }',
    '.lg-buy-btn:hover { transform:translateY(-2px); box-shadow:0 8px 24px rgba(167,139,250,0.3); }',
